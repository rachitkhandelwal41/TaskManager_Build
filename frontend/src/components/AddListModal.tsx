...
import { addNewList } from "../services/taskApi";
...
const AddListModal = ({ setModal }: AddModalProps) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter") submitName(e);
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  });

  async function submitName(event: any) {
    event.preventDefault();
    try {
      const res = await addNewList(name);
      if (res) {
        setModal(false);
        setRefresh(!refresh)
